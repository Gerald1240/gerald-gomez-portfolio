...
onAuthStateChanged(auth, async (user) => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
        // Obtener el documento del trabajador autenticado desde la colección "trabajadores"
        const trabajadorDoc = await getDoc(doc(db, 'trabajadores', user.uid));

        if (trabajadorDoc.exists()) {
            const trabajadorData = trabajadorDoc.data();
            const userAvatar = document.getElementById('user-avatar');

            if (trabajadorData.profileImageUrl && userAvatar) {
                // Cambiar la imagen de perfil si existe en Firestore
                userAvatar.src = trabajadorData.profileImageUrl;
            }
        } else {
            console.log("No se encontró el documento del trabajador");
        }
    } else {
        console.log("No hay usuario autenticado");
    }
